...
class NotificationServiceTest {
  ...
  private static final String TEST_NOTIFICATION_MESSAGE = "Your application status has been updated.";
  private static final String TEMPLATE_NAME = "notification-email.html";
  ...
  void shouldSendNotificationSuccessfully() throws MessagingException {
    ...
    final var notificationEmailDto = new NotificationEmailDto(TEST_NOTIFICATION_MESSAGE, TEST_EMAIL);

    when(javaMailSender.createMimeMessage()).thenReturn(mimeMessage);
    when(templateEngine.process(any(String.class), any(Context.class))).thenReturn(htmlContent);

    notificationService.send(notificationEmailDto);
